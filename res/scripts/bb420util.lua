...
function soundeffectsutil.slow(speed, brakeDecel, maxGain, speed01)
	local gain = 0.0
	if 0.6 < brakeDecel then
		local maxBrakeDecel = 2.5
		local brakeGain = math.sqrt(math.min(brakeDecel / maxBrakeDecel))
		local speed0 = 1.0
		local speed1 = 7.0 * (brakeGain + 1.0)
		local speed2 = 11.0 * (brakeGain + 1.0)
		local speedFadeIn = math.mapClamp(speed, speed1, speed2, 1.0, 0.0)
		local speedFadeOut = math.mapClamp(speed, 0.0, speed0, 0.0, 1.0)
		gain = speedFadeIn * speedFadeOut * brakeGain * maxGain
	end
...
function soundeffectsutil.ebremse(speed, brakeDecel, maxGain, speed01)
	local gain = 0.0
	if 0.8 < brakeDecel then
		local maxBrakeDecel = 2.5
		local brakeGain = math.sqrt(math.min(brakeDecel / maxBrakeDecel))
		local speed0 = 1.0
		local speed1 = 8.0 * (brakeGain + 1.0)
		local speed2 = 20.0 * (brakeGain + 1.0)
		local speedFadeIn = math.mapClamp(speed, speed1, speed2, 1.0, 0.0)
		local speedFadeOut = math.mapClamp(speed, 0.0, speed0, 0.0, 1.0)
		gain = speedFadeIn * speedFadeOut * brakeGain * maxGain
	end
	local pitch = (speed*0.04)+0.5
